...
test('返答完了後に古いキャッシュでスクロール位置へ巻き戻らない', async ({ page }) => {
  await bootstrapChatState(page);
  await installApiMocks(page);
  await page.addInitScript(() => {
    const originalFetch = window.fetch.bind(window);
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      const head = Array.from({ length: 25 }, (_, idx) => `途中 ${idx + 1}`).join('\n');
      const tail = Array.from({ length: 90 }, (_, idx) => `完了後 ${idx + 1}`).join('\n');
      const encoder = new TextEncoder();
      const chunks = [
        JSON.stringify({ type: 'started', turnId: 'turn-scroll-regression-1' }) + '\n',
        JSON.stringify({
          type: 'turn_state',
          seq: 1,
          turnId: 'turn-scroll-regression-1',
          liveReasoningText: '',
          items: [
            {
              id: 'turn-scroll-regression-1:user:0',
              role: 'user',
              type: 'plain',
              text: '巻き戻り確認'
            },
            {
              id: 'turn-scroll-regression-1:assistant:0',
              role: 'assistant',
              type: 'markdown',
              text: head,
              answer: head,
      ...
      const stream = new ReadableStream<Uint8Array>({
        start(controller) {
          const pushNext = () => {
            ...
            controller.enqueue(encoder.encode(chunks[index]));
            index += 1;
            if (index === 3) window.localStorage.setItem('fx:testPhase', 'final');
            setTimeout(pushNext, index === 1 ? 40 : index === 2 ? 120 : index === 3 ? 180 : 40);
          };
          pushNext();
        }
      });
  ...
  await page.goto('/chat/');
  await page.evaluate(() => {
    window.localStorage.setItem('fx:testPhase', 'initial');
  });
  await page.unroute('**/api/threads/messages**');
  await page.route('**/api/threads/messages**', async (route) => {
    const phase = await page.evaluate(() => window.localStorage.getItem('fx:testPhase') || 'initial');
    const items =
      phase === 'final'
        ? [
            {
            ...
            },
            {
              ...
              role: 'assistant',
              type: 'markdown',
              text: Array.from({ length: 90 }, (_, idx) => `完了後 ${idx + 1}`).join('\n'),
              answer: Array.from({ length: 90 }, (_, idx) => `完了後 ${idx + 1}`).join('\n'),
              plan: ''
            }
          ]
        : [];
  ...
  await page.evaluate(() => {
    window.localStorage.setItem(
      'fx:threadMessages:thread-e2e-1',
      JSON.stringify([
        { id: 'stale-user-1', role: 'user', type: 'plain', text: '古い会話' },
        { id: 'stale-assistant-1', role: 'assistant', type: 'markdown', text: '古い返答', answer: '古い返答', plan: '' }
      ])
    );
  });

  await page.getByTestId('composer-textarea').fill('巻き戻り確認');
  await page.getByTestId('send-button').click();

  await expect(page.locator('.fx-msg-assistant .fx-msg-bubble')).toContainText('完了後 90');
  await expect(page.getByText('古い会話')).toHaveCount(0);
  await expect(page.getByTestId('stream-loading-indicator')).toHaveCount(0);
  await expect
    .poll(async () =>
      page.evaluate(() => {
        const container = document.querySelector('.fx-chat-scroll');
