...
test('保存済みのスクロール位置があればチャット表示時にそれを優先する', async ({ page }) => {
  await bootstrapChatState(page);
  await page.addInitScript(() => {
    window.localStorage.setItem('fx:threadScrollTopByThread', JSON.stringify({ 'thread-e2e-1': 240 }));
  });
  await installApiMocks(page);
...
test('チャットを離れて戻った時は保存したスクロール位置へ戻る', async ({ page }) => {
  await bootstrapChatState(page);
  await installApiMocks(page);
