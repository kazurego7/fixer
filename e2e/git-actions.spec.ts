...
test('Git アイコン押下で Codex にコミットと push を依頼できる', async ({ page }) => {
  ...
  await expect(page.getByTestId('workspace-tab-files')).toHaveAttribute('data-status', 'updated');
...
test('Git が同期済みのときはコミットと push ボタンを無効化する', async ({ page }) => {
  ...
  await expect(page.getByTestId('workspace-tab-files')).toHaveAttribute('data-status', 'idle');
