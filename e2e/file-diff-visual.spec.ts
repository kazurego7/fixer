...
test('差分詳細画面の見た目をスクリーンショットで確認できる', async ({ page }, testInfo) => {
  ...
  const changedPath = testInfo.outputPath('file-diff-changed.png');
  await page.screenshot({ path: changedPath, fullPage: false });
  await testInfo.attach('file-diff-changed', {
    path: changedPath,
  ...
  const nextPath = testInfo.outputPath('file-diff-next.png');
  await page.screenshot({ path: nextPath, fullPage: false });
  await testInfo.attach('file-diff-next', {
    path: nextPath,
    contentType: 'image/png'
  });
  ...
  const prevPath = testInfo.outputPath('file-diff-prev.png');
  await page.screenshot({ path: prevPath, fullPage: false });
  await testInfo.attach('file-diff-prev', {
    path: prevPath,
    contentType: 'image/png'
  });
  ...
  const unchangedPath = testInfo.outputPath('file-diff-unchanged.png');
  await page.screenshot({ path: unchangedPath, fullPage: false });
  await testInfo.attach('file-diff-unchanged', {
    path: unchangedPath,
    contentType: 'image/png'
  });
  ...
  const ignoredPath = testInfo.outputPath('file-diff-ignored.png');
  await page.screenshot({ path: ignoredPath, fullPage: false });
  await testInfo.attach('file-diff-ignored', {
    path: ignoredPath,
    contentType: 'image/png'
  });
});
