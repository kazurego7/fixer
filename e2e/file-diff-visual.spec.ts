...
test('差分詳細画面の見た目をスクリーンショットで確認できる', async ({ page }, testInfo) => {
  ...
  await page.route('**/api/repos/file-view?**path=assets%2Flogo.png**', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
        ...
        mimeType: 'image/png',
        imageDataUrl:
          'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO6p8dQAAAAASUVORK5CYII='
      })
    });
  });
  ...
  await saveVisualScreenshot(page, testInfo, 'file-diff-image.png', {
    attachmentName: 'file-diff-image'
  });
});
