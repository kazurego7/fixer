...
test('最新プランがある場合のみプラン実現ボタンを表示する', async ({ page }) => {
  ...
  await expect(page.locator('.fx-chat-head [data-testid="plan-apply-button"]')).toHaveCount(0);
  await expect(page.locator('.fx-chat-scroll [data-testid="plan-apply-button"]')).toHaveCount(1);
});
